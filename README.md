...
核心目标是提高等权Top-5组合的样本外未来5日收益与超额收益。全体候选股
用于提供横截面监督和负样本，但不再要求精确排列约300只股票的全部名次；
Rank IC保留为防止稳定负相关和分数塌缩的辅助诊断。

训练与推理主流程如下：
1. 新实验读取时点真实数据（`data/hs300_pit/model_data.csv`）；旧
   `data/stock_data.csv`只用于历史复现；
2. 做特征工程（39特征或`158+39`特征）；
3. 构建标签：信号日次日开盘到第5个统一市场交易日收盘的可执行收益；
4. 按“日期”组织排序样本：每个样本是一日内多只股票的序列与目标；
5. 训练头部导向排序模型，以验证集 `top5_return`（等权比赛得分）早停并保存最优权重；
...
- 损失函数：`HeadFocusedRankingLoss`
	- 主项比较真实Top-5与其余股票；
	- 小权重保留全横截面排序和负相关稳定性约束。
- 评估指标：`calculate_ranking_metrics()`
	- 计算Top-5/10/20收益及超额收益、NDCG@5、头部真实百分位；
	- Rank IC仅作为辅助泛化诊断；
	- 比赛阶段以验证集Top-5等权原始收益选择最优模型，超额收益仅作诊断。
