...
def calculate_ranking_metrics(y_pred, y_true, masks, k=5):
    ...
    # 评估器按每日横截面返回均值；外层月度汇总需要知道实际有效交易日数。
    metrics['_n_days'] = len(pred_return_sum_list)
    return metrics


def create_predict_style_evaluation_dataset(
    raw_df, stockid2idx, features, scaler, start_date, end_date,
):
    """Build labelled evaluation cross-sections through the ``predict.py`` path.

    Candidate selection and sequence construction intentionally reuse
    ``predict.py`` helpers.  Labels are joined only after the complete
    prediction universe is fixed, so a test label can never decide whether a
    stock is scored.  A partially unlabelled candidate pool is rejected rather
    than silently shrinking the universe used for IC/NDCG/Top-5 metrics.  A
    wholly unlabelled terminal date is omitted as an unavailable label horizon.
    """
    processed, available_features = preprocess_predict_data(raw_df, stockid2idx)
    missing_features = set(features).difference(available_features)
    if missing_features:
        raise ValueError(
            'predict.py特征处理缺少训练特征: '
            f'{sorted(missing_features)}'
        )
    processed = prepare_predict_features(processed, features, scaler)

    label_source = raw_df.copy()
    label_source['日期'] = pd.to_datetime(label_source['日期']).dt.normalize()
    if config.get('label_mode') == 'contest_score':
        label_source = apply_contest_score_label(label_source)
    elif 'label' not in label_source.columns:
        raise ValueError('预测式评估需要真实标签列')
    labels = label_source[['股票代码', '日期', 'label']].copy()
    labels['股票代码'] = labels['股票代码'].astype(str).str.zfill(6)
    labels['label'] = pd.to_numeric(labels['label'], errors='coerce')
    # 原始PIT面板可能自带旧label；评估只使用此处按当前口径重建的标签。
    processed = processed.drop(columns=['label'], errors='ignore')
    ...
    dates = sorted(
        pd.DatetimeIndex(processed.loc[
            processed['日期'].between(start_date, end_date), '日期'
        ].dropna().unique()).normalize()
    )
    sequences = []
    targets = []
    relevance_scores = []
    ...
    for signal_date in dates:
        candidate_ids = get_predict_candidate_ids(processed, signal_date)
        sequence_values, scored_stock_ids = build_inference_sequences(
            processed,
            features,
            sequence_length,
            candidate_ids,
        ...
        signal_rows = processed.loc[
            processed['日期'].eq(signal_date), ['股票代码', 'label']
        ].set_index('股票代码')['label']
        target_values = signal_rows.reindex(scored_stock_ids).to_numpy(
            dtype=np.float32
        )
        missing_labels = np.asarray(~np.isfinite(target_values))
        if missing_labels.all():
            # E.g. the final five signal days in July: no stock has a complete
            # future five-day label, so the day is not an evaluable test day.
            continue
        if missing_labels.any():
            missing_stock_ids = np.asarray(scored_stock_ids)[missing_labels]
            raise ValueError(
                '预测式评估拒绝缩小完整候选池：'
                f'{signal_date.date()}有{int(missing_labels.sum())}只已评分股票缺少标签，'
                f'例如{missing_stock_ids[:5].tolist()}'
            )
        if len(scored_stock_ids) < 10:
            raise ValueError(
                f'预测式评估候选池不足10只股票: {signal_date.date()} '
                f'({len(scored_stock_ids)})'
            )
        sequences.append(sequence_values)
        targets.append(target_values)
        relevance_scores.append(returns_to_relevance(target_values))
        stock_indices.append([
            stockid2idx[stock_id] for stock_id in scored_stock_ids
        ])
        sample_dates.append(pd.Timestamp(signal_date))
        universe_audit.append({
            'signal_date': pd.Timestamp(signal_date).strftime('%Y-%m-%d'),
            'eligible_before_sequence': int(len(candidate_ids)),
            'scored_stocks': int(len(scored_stock_ids)),
            'labelled_scored_stocks': int(len(target_values)),
        })

    dataset = RankingDataset(
        sequences, targets, relevance_scores, stock_indices,
    )
    return dataset, sample_dates, universe_audit
...
def evaluate_ranking_model(
    ...
    full_ranking_output=None, sample_dates=None, stockid2idx=None,
):
    """Evaluate daily cross-sections and optionally persist every raw score.

    ``full_ranking_output`` is reserved for held-out evaluation.  Its rows are
    written directly from the model output before Top-5 selection, preserving
    the complete prediction universe used by the metrics.
    """
    model.eval()
    total_loss = 0
    total_metrics = {}
    ...
    sample_offset = 0
    idx2stock = (
        {int(index): stock_id for stock_id, index in stockid2idx.items()}
        if stockid2idx is not None else None
    )
    if full_ranking_output and (sample_dates is None or idx2stock is None):
        raise ValueError('导出完整预测排序需要sample_dates和stockid2idx')
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"Evaluating Epoch {epoch+1}"):
            ...
            if full_ranking_output:
                batch_stock_indices = batch['stock_indices']
                for i in range(sequences.size(0)):
                    signal_date = pd.Timestamp(
                        sample_dates[sample_offset + i]
                    ).strftime('%Y-%m-%d')
                    valid_positions = torch.nonzero(
                        masks[i].bool(), as_tuple=False
                    ).squeeze(1)
                    raw_scores = outputs[i, valid_positions].detach().cpu().numpy()
                    raw_targets = targets[i, valid_positions].detach().cpu().numpy()
                    raw_stock_indices = batch_stock_indices[
                        i, valid_positions.detach().cpu()
                    ].tolist()
                    order = np.argsort(raw_scores)[::-1]
                    for rank, position in enumerate(order, start=1):
                        ranking_rows.append({
                            'signal_date': signal_date,
                            'stock_id': idx2stock[int(raw_stock_indices[position])],
                            'raw_score': float(raw_scores[position]),
                            'raw_rank': int(rank),
                            'future_return': float(raw_targets[position]),
                            'selected_top5': bool(rank <= 5),
                            'weight': 0.2 if rank <= 5 else 0.0,
                        })
            sample_offset += sequences.size(0)
            ...
            # 计算评估指标
            metrics = calculate_ranking_metrics(masked_outputs, masked_targets, masks, k=5)
            metric_days = int(metrics.pop('_n_days', 0))
            if metric_days == 0:
                num_batches += 1
                continue
            gate_values = getattr(model, 'last_gate_values', None)
            ...
            for k, v in metrics.items():
                if k not in total_metrics:
                    total_metrics[k] = 0
                total_metrics[k] += v * metric_days
            total_metric_days += metric_days
    ...
    for k in total_metrics:
        total_metrics[k] /= total_metric_days if total_metric_days else 1
    total_metrics['_n_days'] = int(total_metric_days)
    ...
    if full_ranking_output:
        if sample_offset != len(sample_dates):
            raise RuntimeError(
                '完整预测排序的日期数与评估数据集不一致: '
                f'{sample_offset} != {len(sample_dates)}'
            )
        output_dir = os.path.dirname(full_ranking_output)
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)
        pd.DataFrame(ranking_rows).to_csv(
            full_ranking_output, index=False, encoding='utf-8-sig'
        )
...
def main():
    ...
    # 验证与测试指标均通过predict.py的候选池、序列和特征处理路径构建。
    # 标签仅在完整预测股票池被确定之后接入，不能反向影响候选股票。
    val_dataset, val_sample_dates, val_universe_audit = (
        create_predict_style_evaluation_dataset(
            full_df, stockid2idx, features, scaler, val_start, val_end,
        )
    )
    with open(
        os.path.join(output_dir, 'evaluation_validation_universe.json'),
        'w', encoding='utf-8',
    ) as handle:
        json.dump(
            {
                'data_source': 'predict.py_complete_candidate_universe',
                'days': val_universe_audit,
            }, handle, indent=2, ensure_ascii=False,
        )
    has_held_out_test = (
        not competition_5day and split['test']['n_days'] > 0
    )

    print(f"训练集样本数: {len(train_sequences)}")
    print(f"{'比赛信号日' if competition_5day else '验证集'}预测式样本数: {len(val_sample_dates)}")
    ...
    if is_train:
        if competition_5day:
            ...
            test_loss, test_metrics = evaluate_ranking_model(
                ...
                writer,
                config['num_epochs'] - 1,
                full_ranking_output=os.path.join(
                    output_dir, 'evaluation_competition_5day_full_ranking.csv'
                ),
                sample_dates=val_sample_dates,
            ...
            evaluation = {
                ...
                'full_ranking_file': 'evaluation_competition_5day_full_ranking.csv',
                    ...
                    json.dump(
                        {
                            ...
                            'metric_data_source': (
                                'predict.py_complete_candidate_universe'
                            ),
                            'universe_audit_file': (
                                'evaluation_validation_universe.json'
                            ),
                            'evaluated_validation_dates': [
                                pd.Timestamp(sample_date).strftime('%Y-%m-%d')
                                for sample_date in val_sample_dates
                            ],
                            'metrics': {
                                key: float(value)
        ...
        if has_held_out_test and config.get('refit_after_validation', False):
            ...
            refit_date_values = pd.to_datetime(
                processed_unscaled['日期']
            ).dt.normalize()
            if split.get('gap1', {}).get('start'):
                refit_gap1_rows = (
                    (refit_date_values >= pd.to_datetime(split['gap1']['start']))
                    & (refit_date_values <= pd.to_datetime(split['gap1']['end']))
                )
            else:
                refit_gap1_rows = pd.Series(
                    False, index=processed_unscaled.index
                )
            refit_rows = (
            ...
            # 保留gap1作为序列上下文，但禁止其未来标签进入refit目标。
            refit_processed.loc[refit_gap1_rows, 'label'] = np.nan
            ...
            (
                ...
                refit_stock_indices, refit_train_sample_dates,
            ) = refit_result
            refit_train_date_strings = [
                pd.Timestamp(sample_date).strftime('%Y-%m-%d')
                for sample_date in refit_train_sample_dates
            ]
            with open(
                os.path.join(output_dir, 'refit_target_dates.json'),
                'w', encoding='utf-8',
            ) as handle:
                json.dump(
                    {
                        'n_days': len(refit_train_date_strings),
                        'dates': refit_train_date_strings,
                        'excluded_gap1': split.get('gap1'),
                    },
                    handle, indent=2, ensure_ascii=False,
                )
            ...
            config['refit_excluded_gap1'] = split['gap1']
            config['refit_train_target_days'] = int(
                len(refit_train_sample_dates)
            )
            with open(
            ...
            # 最终模型训练完毕后才读取测试标签，并从predict.py完整候选池评估。
            refit_test_dataset, refit_test_sample_dates, refit_test_universe = (
                create_predict_style_evaluation_dataset(
                    full_df,
                    stockid2idx,
                    features,
                    refit_scaler,
                    pd.to_datetime(split['test']['start']),
                    pd.to_datetime(split['test']['end']),
                )
            )
            with open(
                os.path.join(output_dir, 'evaluation_test_universe.json'),
                'w', encoding='utf-8',
            ) as handle:
                json.dump(
                    {
                        'data_source': 'predict.py_complete_candidate_universe',
                        'days': refit_test_universe,
                    }, handle, indent=2, ensure_ascii=False,
                )
            ...
            test_loss, test_metrics = evaluate_ranking_model(
                ...
                best_epoch,
                full_ranking_output=os.path.join(
                    output_dir, 'evaluation_test_full_ranking.csv'
                ),
                sample_dates=refit_test_sample_dates,
                stockid2idx=stockid2idx,
            )
            refit_performed = True
        elif has_held_out_test:
            checkpoint_path = os.path.join(output_dir, 'best_model.pth')
            model.load_state_dict(torch.load(checkpoint_path, map_location=device))
            test_dataset, test_sample_dates, test_universe = (
                create_predict_style_evaluation_dataset(
                    full_df,
                    stockid2idx,
                    features,
            ...
            test_loss, test_metrics = evaluate_ranking_model(
                ...
                sample_dates=test_sample_dates,
                stockid2idx=stockid2idx,
            )

        if test_metrics is not None:
            evaluated_test_dates = (
                refit_test_sample_dates if refit_performed else test_sample_dates
            )
            evaluated_test_date_strings = [
                pd.Timestamp(sample_date).strftime('%Y-%m-%d')
                for sample_date in evaluated_test_dates
            ]
            evaluation_test = {
                'protocol': (
                ...
                'evaluated_test_start': (
                    evaluated_test_date_strings[0]
                    if evaluated_test_date_strings else None
                ),
                'evaluated_test_end': (
                    evaluated_test_date_strings[-1]
                    if evaluated_test_date_strings else None
                ),
                'evaluated_test_days': len(evaluated_test_date_strings),
                'evaluated_test_dates': evaluated_test_date_strings,
                'metric_data_source': 'predict.py_complete_candidate_universe',
                'full_ranking_file': 'evaluation_test_full_ranking.csv',
                'universe_audit_file': 'evaluation_test_universe.json',
                'refit_train_target_days': (
                    len(refit_train_sample_dates) if refit_performed else None
                ),
                'test_loss': float(test_loss),
                'metrics': {
