...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument(
		"--input",
		type=str,
		default="data/hs300_pit/model_data.csv",
		help="原始数据文件路径，默认 data/hs300_pit/model_data.csv",
	)
	parser.add_argument(
		"--output-dir",
		type=str,
		default="data/hs300_pit/splits",
		help="输出目录，默认 data/hs300_pit/splits（不覆盖旧实验切分）",
	)
	parser.add_argument(
		"--train-start",
		type=str,
...
def main() -> None:
	...
	if "in_signal_range" in df.columns:
		signal_mask = df["in_signal_range"].astype(str).str.lower().isin({"true", "1"})
